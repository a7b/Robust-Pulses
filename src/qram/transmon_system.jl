"""
transmon_system.jl - sets up single transmon system for the control qubit in the QRAM system
"""
...
#test comment
# paths
const TRANSMON_OUT_PATH = abspath(joinpath(WDIR, "out", "qram_single_transmon"))

# experimental constants
const ω_q = 0 #2π * 4.96 #GHz
const α = 2π * 0.10033 #GHz
...
# multiply by negative imaginary number because that is what 
# goes in the matrix exponential time evolution operator
# get_mat_iso converts C^n matrices to their R^2n isomorphic representation
const NEGI_TRANSMON_NUMBER_ISO = get_mat_iso(-1im * TRANSMON_NUMBER)
#identity operator 
const TRANSMON_ID = I(TRANSMON_STATE_COUNT)

const TRANSMON_QUAD = TRANSMON_CREATE * TRANSMON_CREATE * TRANSMON_ANNIHILATE * TRANSMON_ANNIHILATE
...
#bare drift hamiltonian
const NEGI_H0_ISO = get_mat_iso(
    -1im * (-α/2 * TRANSMON_QUAD)
)

const NEGI_H1R_ISO = get_mat_iso(
    -1im * (TRANSMON_CREATE + TRANSMON_ANNIHILATE)/2
)
const NEGI_H1I_ISO = get_mat_iso(
    -1im * (1im/2 * (TRANSMON_CREATE - TRANSMON_ANNIHILATE))
)
# gates
...
#= 
To do a unitary with Altro we need to set up
the state transfer problem using the operator basis
to make sure we get all the relative phases right.
=# 
# |g⟩
...
# target states after X pi pulse
...
function hessian_gate_error_iso2(s2::AbstractVector; s2o::Int64=0)
