...
function run_traj(;evolution_time=40., solver_type=altro,
    ...
    return (return_cost(data_path),data_path)
end
...
space = [ho.hp.loguniform("deriv", -2, 9), ho.hp.loguniform("state", -2, 9)]
tpe_alg = ho.tpe.suggest
tpe_trials = ho.Trials()

py"""
from hyperopt import STATUS_OK
def make_dict(toterror, deriv, controls, fp):
    return {'loss': toterror, 'status': STATUS_OK, 'deriv': deriv, 'controls': controls, 'fp': fp}
"""
