...
const EXPERIMENT_NAME = "transmon_pi_g"
...
function run_traj(;evolution_time=16, solver_type=altro,
    ...
    Q = Diagonal(SVector{n}([
        ...
        fill(qs[5], eval(:($derivative_order >= 2 ? $SAMPLE_COUNT * $HDIM_ISO : 0))); # ∂2ψ
    ]))
    Qf = Q * N
...
function plot_population(save_file_path; title="Population vs Time", xlabel="Time (ns)", ylabel="Population",
                         legend=:topright)
    # grab
