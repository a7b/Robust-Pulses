...
const EXPERIMENT_NAME = "transmon_only1e"
...
function run_traj(;evolution_time=16, solver_type=altro,
    ...
    xf_ = zeros(n)
    xf_[STATE1_IDX] = XPIBY2_ISO
    xf_[STATE2_IDX] = XPIBY2_ISO2
    ...
    Q = Diagonal(SVector{n}([
        ...
        #                * $HDIM_ISO : 0))) # ψ3, ψ4
        # fill(0, HDIM_ISO);
        fill(qs[4], eval(:($derivative_order >= 1 ? $SAMPLE_COUNT * $HDIM_ISO : 0)));
         # eval(:($derivative_order >= 1 ? $SAMPLE_COUNT * $HDIM_ISO : 0))); # ∂ψ
        fill(qs[5], eval(:($derivative_order >= 1 ? $SAMPLE_COUNT * $HDIM_ISO : 0)));
...
function gen_dparam(save_file_path; trial_count=500, sigma_max=1e-4, save=true)
    ...
    ψT2 = XPIBY2_subspace[:,2]
    xf[STATE1_IDX] = XPIBY2_ISO
    xf[STATE2_IDX] = XPIBY2_ISO2
    # generate parameters
    ωq_dev_max = 2π * 2e-3
